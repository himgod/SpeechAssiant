...
public class SematicAnalyzerPutian implements SematicAnalyzer{
	
	int index;
	List<WordResult> tokenList;

	public SematicAnalyzerPutian(){
		
	}
	
	public SematicAnalyzerPutian(List<WordResult> list){
		tokenList = list;
		index = 0;
	}
	...
	private void executeMinimizeCommand(){
		...
		}else if("全部".equals(word.getSpelling())){
			word = getNextWord();
	...
	public void executeQuery(){
		...
		if("天气预报".equals(word.getSpelling())){
			
			opener.openUrl("https://www.baidu.com/s?wd=天气预报");
		}
		
		if("新历书".equals(word.getSpelling())){
	...
	public void execute(){
		...
		while((word = getNextWord())!=null && !word.isSentenceEndWord()){
			if("开启".equals(word.getSpelling())){
				executeOpenCommand();
			}else if("退出".equals(word.getSpelling())){				
				executeCloseCommand();
			...
			}else if("查".equals(word.getSpelling())){
				executeQuery();
			}else if("清空".equals(word.getSpelling())){
				executeClean();
			}else if("找".equals(word.getSpelling())){
				executeLocalSearch();
			...
			}else if("点".equals(word.getSpelling())){
				word = getNextWord();
